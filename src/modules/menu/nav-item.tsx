      ...
      <ul>
        {item.children?.map((grandChild) => (
          <li className="text-sm" key={grandChild.id}>
            <a className="font-normal !text-zinc-700 hover:text-blue-500" href={grandChild.to} key={grandChild.id}>
              {grandChild.label}
            </a>
          </li>
        ))}
  ...
  return (
    <li className="group relative h-full">
      <a
        href={item.to}
        key={item.id}
        className="flex size-full items-center gap-1 p-2 text-lg font-semibold text-white transition hover:!text-blue-500"
      >
        {item.label}
