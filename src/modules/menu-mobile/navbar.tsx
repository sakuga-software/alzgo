  ...
  return (
    <li className="group text-black" data-open={Boolean(isOpen)}>
      ...
      >
        ...
        <Icon
          name="chevron-right"
          className="text-blue-500 ml-auto size-10 rotate-0 transition duration-200 group-data-[open=true]:rotate-90"
        />
      </button>
      ...
      <ul className="transition-all group-data-[open=false]:h-0 group-data-[open=false]:overflow-hidden group-data-[open=true]:!m-8">
        {child.children?.map((grandchild) => (
          <li key={grandchild.id} className="mb-3 text-base">
            <a className='!text-black' href={grandchild.to}>{grandchild.label}</a>
          </li>
        ))}
      </ul>
    </li>
  ...
  return (
    <>
      <button className="m-auto flex scale-150 flex-col gap-1" onClick={() => setIsOpen((prev) => !prev)} type="button">
        <span
          className={cn(`bg-blue-500 block h-1 w-6 transition-transform`, isOpen && 'translate-y-2 rotate-45')}
        />
        <span className={cn(`bg-blue-500 block h-1 w-6 transition-opacity`, isOpen && 'opacity-0')} />
        <span
          className={cn(`bg-blue-500 block h-1 w-6 transition-transform`, isOpen && '-translate-y-2 -rotate-45')}
        />
      </button>
            ...
            {items?.map((item) => (
              <React.Fragment key={item.id}>
                <li className="group/item h-20 !text-black" data-open={Boolean(openOpenItems[item.id])}>
                  ...
                  >
                    <p className="p-0 m-0 font-semibold">{item.label}</p>
                    {item.children && item.children.length > 0 && (
                      <Icon
                        name="chevron-right"
                        className="text-blue-500 ml-auto size-10 transition duration-200 group-data-[open=true]/item:rotate-180"
                      />
                    )}
                  </a>
                </li>
                ...
                {item.children && item.children.length > 0 && (
                  <div
                    className="group/item _data-[open=false]:h-0 _data-[open=false]:overflow-hidden text-blue-950 absolute top-0 left-0 z-10 w-full transform bg-white transition-all duration-200 data-[open=false]:w-0 data-[open=false]:-translate-x-16 data-[open=false]:opacity-0"
                  ...
                  >
                    ...
                    >
                      <Icon
                        name="chevron-right"
                        className="text-blue-500 size-10 transition duration-200 group-data-[open=true]/item:rotate-180"
                      />
